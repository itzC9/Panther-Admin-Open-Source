...
if not getconnections or not hookmetamethod or not getnamecallmethod or not ((getgenv and getgenv()) or _G) then
	game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Destroyer Admin",Text = "Executor is not supported!",Duration = 10,})
end
if not workspace:FindFirstChild("Criminals Spawn") or not workspace:FindFirstChild("Criminals Spawn"):FindFirstChild("SpawnLocation") then
	game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Destroyer Admin",Text = "Criminals spawn not found! Please rejoin.",Duration = 10,})
end
game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart")
if game:FindFirstChild("Destroyer_loaded") then  ((getgenv and getgenv()) or _G).NotifDestroyer("Destroyer Admin is already executed!",false) return warn("Already loaded") end
...
CmdBarFrame.Position = UDim2.new(0, 0, 0, 0)
...
do
	...
	ToggleScroll.BackgroundTransparency = 0
	...
	tempb.BackgroundTransparency = 0
	tempb.Position = UDim2.new(0, 0, 0, 0)
...
CommandsList.BackgroundTransparency = 0
...
TEMP_CMD.BackgroundTransparency = 0
...
function API:CreateCmd(Header, Description, Callback,IsHide,Extra,IsPre,plsdonotlower)
	...
	local function FactCheck(TheText)
		...
		if First:lower() == Header:lower() or First:lower() == Prefix..Header:lower() then
			...
			if not a and b then
				warn("--> Destroyer Admin DETECTED AN ERROR")
...
GlobalVar.NotifDestroyer = function(t,v)
	API:Notif(t,v)
end
local States = {}
local Settings = {
...
if writefile and makefolder and readfile and isfile then
	if isfile("Destroyer Admin") == false or isfile("Destroyer_Admin/Invite.json") == false then
		makefolder("Destroyer_Admin")
		if isfile("Destroyer_Admin/Invite.json") == false then
			writefile("Destroyer_Admin/Invite.json",game:GetService("HttpService"):JSONEncode({
				Invite_To_Server = true
			}))
		end
		if isfile("Destroyer_Admin/SavedAdmins.json") == false then
			writefile("Destroyer_Admin/SavedAdmins.json",game:GetService("HttpService"):JSONEncode({}))
		else
			local Content = game:GetService("HttpService"):JSONDecode(readfile("Destroyer_Admin/SavedAdmins.json"))
...
task.spawn(function()
	if writefile and makefolder and readfile and isfile then
		if game:GetService("HttpService"):JSONDecode(readfile("Destroyer_Admin/Invite.json")).Invite_To_Server then
...
--[[
		...
		API:CreateCmd("kick", "!KICKS Destroyer Admin USERS ONLY! SAY IN CHAT", function(args)
		end,nil,"[Destroyer Admin USER]",true)
		...
		API:CreateCmd("anticrash", "Tries to stop simple crashes (DOESNT WORK WITH Destroyer Admin CRASH)", function(args)
		...
		API:CreateCmd("autoadmin", "Admins someone even if you leave and rejoin", function(args)
			...
			if Target then
				...
				if not Found then
					...
					local ohString2 = "All"
					game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(ohString1, ohString2)
					writefile("Destroyer_Admin/SavedAdmins.json",game:GetService("HttpService"):JSONEncode(New))
	...
	--//end of premium
	API:CreateCmd("drag", "Enables draggable mode for all Destroyer guis", function(args)
	...
	API:CreateCmd("noinvite", "disables discord invite", function(args)
		if writefile and readfile then
			writefile("Destroyer_Admin/Invite.json",game:GetService("HttpService"):JSONEncode({Invite_To_Server=false}))
	...
	API:CreateCmd("allcmds", "tells you the ammount of commands Destroyer Admin has", function(args)
		API:Notif("Destroyer Admin has "..tostring(AmmountCurrent).." commands.")
	end)
	...
	API:CreateCmd("unload", "Destroys the script unloading it", function(args)
		API:Destroy(game:FindFirstChild("Destroyer__loaded"))
		Unloaded = true
...
Title.Text = "Destroyer Admin"
Title.TextColor3 = Color3.fromRGB(255, 255, 255)
Title.TextSize = 24

CommandBar.Parent = Frame
CommandBar.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
CommandBar.Position = UDim2.new(0, 0, 1, -50)  -- Positioned at the bottom
...
ScrollingFrame.Position = UDim2.new(0, 0, 0.2, 0)  -- Adjusted height to make room for the CommandBar
ScrollingFrame.Size = UDim2.new(0, 300, 0, 190)  -- Adjusted height to make room for the CommandBar
ScrollingFrame.CanvasSize = UDim2.new(0, 0, 5, 0)
ScrollingFrame.ScrollBarThickness = 10

-- Function to add command labels to the scrolling frame
local function addCommandLabel(commandText)
    local commandLabel = Instance.new("TextLabel")
    commandLabel.Parent = ScrollingFrame
    commandLabel.BackgroundColor3 = Color3.fromRGB(75, 75, 75)
    commandLabel.Size = UDim2.new(0, 280, 0, 50)
    commandLabel.Position = UDim2.new(0, 10, 0, (#ScrollingFrame:GetChildren() - 1) * 55)
...
-- Function to add command labels
local function addCommandLabel(command)
    -- Your logic for adding command labels
end

-- List of commands
local commands = {
    "crimbase", "roof", "tower", "cafe", "yard", "cells", "kitchen", "secretroom", "armory", "office",
    "lobby", "exploitersbase", "gate", "guardtower", "Bring [Plyr]", "kill [Plr, Team]", "Arrest All",
    "Tase [All, Plyr]", "loopkill [Plyr, Team, all]", "looptase [Plyr, All]", "AntiTase [On/Off]", "GodMode",
    "Trap [Plyr]", "modgun", "rapidfire", "infammo", "modgun", "guns", "mkill", "loopmkill", "unfly", "vfly",
    "removecars", "doors", "opendoors", "loopopendoors", "anticrash", "lag"
}
...
local function executeCommand(command)
    command = command:lower()  -- Convert command to lowercase for case insensitivity

    -- Example Commands
    if command == "crimbase" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-944, 94, 2057)
    elseif command == "tower" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(783, 125, 2590)
    elseif command == "roof" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(824, 119, 2353)
    elseif command == "cafe" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(875, 100, 2251)
    elseif command == "yard" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(919, 100, 2455)
    elseif command == "cells" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(919, 100, 2455)
    elseif command == "kitchen" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(894, 102, 2225)
    elseif command == "secretroom" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(698, 100, 2374)
    elseif command == "armory" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(840, 100, 2266)
    elseif command == "office" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(794, 100, 2297)
    elseif command == "lobby" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(876, 27, 2347)
    elseif command == "exploitersbase" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-937, 94, 1918)
    elseif command == "gate" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(481, 98, 2216)
    elseif command == "guardtower" then
        LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(501, 143, 2069)
    
    -- Bring, Kill, Arrest, and Tase commands (implement these accordingly)
    elseif command:match("^bring") then
        -- Implementation for bring command
    elseif command:match("^kill") then
        -- Implementation for kill command
    elseif command == "arrest all" then
        -- Implementation for arrest all command
    elseif command:match("^tase") then
        -- Implementation for tase command
    elseif command:match("^loopkill") then
        -- Implementation for loopkill command
    elseif command:match("^looptase") then
        -- Implementation for looptase command
    elseif command:match("^antitase") then
        -- Implementation for antitase command
    
    -- Utility commands
    elseif command == "godmode" then
        -- Implementation for godmode command
    elseif command:match("^trap") then
        -- Implementation for trap command
    
    -- Weapon commands
    elseif command == "modgun" then
        -- Implementation for modgun command
    ...
    elseif command == "guns" then
        -- Implementation for giving all guns
    elseif command == "mkill" then
        -- Implementation for mkill command
    elseif command == "loopmkill" then
        -- Implementation for loopmkill command
    
    -- Fly commands
    elseif command == "unfly" then
        -- Implementation for unfly command
    elseif command == "vfly" then
        -- Implementation for vfly command
    
    -- Door commands
    elseif command == "removecars" then
        local Old = API:GetPosition()
        for i, v in pairs(game:GetService("Workspace").CarContainer:GetChildren()) do
            if v then
                repeat task.wait() until LocalPlayer.Character:FindFirstChildOfClass("Humanoid").Health > 1
                local car = v
                if car:FindFirstChild("RWD") and car:FindFirstChild("Body") and car:FindFirstChild("Body"):FindFirstChild("VehicleSeat").Occupant == nil then
                    local Seat = car.Body.VehicleSeat
                    car.PrimaryPart = car.RWD
                    ...
                    until LocalPlayer.Character:FindFirstChildOfClass("Humanoid").Sit == true
                    for i = 1, 5 do
                        wait()
                        car:SetPrimaryPartCFrame(Old)
                    end
    ...
    elseif command == "opendoors" then
        for i, v in pairs(game:GetService("Workspace").Doors:GetChildren()) do
            if v then
                v:FindFirstChild("Open"):FireServer()
            end
        end
    elseif command == "loopopendoors" then
        while wait(5) do
            for i, v in pairs(game:GetService("Workspace").Doors:GetChildren()) do
                if v then
                    v:FindFirstChild("Open"):FireServer()
    ...
    elseif command == "anticrash" then
        -- Implementation for anti-crash
    elseif command == "lag" then
        while wait(0.1) do
            game:GetService("RunService").Stepped:Wait()
        end
    else
        API:Notif("Invalid command!", false)
    end
end

-- Connect the command bar to execute commands on enter
CommandBar.FocusLost:Connect(function(enterPressed)
    if enterPressed then
        executeCommand(CommandBar.Text)
        CommandBar.Text = ""  -- Clear the command bar after execution
    end
end)
...
function AdminChatted(Text,Speaker)
	...
	if PlayerCheckCommand("killaura") then
	...
	if PlayerCheckCommand("killaura off") then
